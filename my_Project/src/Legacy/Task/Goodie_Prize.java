...
class GoodiePrice
{
  ...
  public GoodiePrice(String name, int price) 
  {
    this.name = name;
    this.price = price;
  }
...
public class Goodie_Prize	
{
  public static void main(String[] args) throws Exception 
  {
    ...
    ArrayList<GoodiePrice> goodies_items = new ArrayList<GoodiePrice>();

    while(sc.hasNextLine())  
    {
      String current[] = sc.nextLine().split(": ");
      goodies_items.add(new GoodiePrice(current[0], Integer.parseInt(current[1])));
    }
    sc.close();

    Collections.sort(goodies_items, new Comparator<GoodiePrice>()
    {
      public int compare(GoodiePrice a, GoodiePrice b) 
      { 
